...
def main_frame():
    ...
    string_op = Button(f, text="Enter a String", height=2, width=25, bd=5,
    bg="white",
    ...
    title1 = Label(f, text="CRYPTOGRAPHY", font=('Snap ITC', 40),bg="IndianRed1")
...
def string_frame():
    ...
    EncryptButton = Button(frame_string_op, text="ENCRYPT", width=18,height=2, bd=5, bg="green3",font=('Lucida Bright', 18, 'bold'),command=lambda: encrypt_label())
    DecryptButton = Button(frame_string_op, text="DECRYPT", width=18,height=2, bd=5, bg="green3",font=('Century Schoolbook', 18, 'bold'),command=lambda: decrypt_label())
    ExitButton = Button(frame_string_op, text="EXIT", bg="red", height=2,width=10, bd=5,font=('Century Schoolbook', 18, 'bold'),command=lambda: raise_frame(main_frame(),frame_string_op))
    msg1 = Label(frame_string_op, text="Enter a string :", font=('LucidaBright', 25, 'bold'), bg="AntiqueWhite1")
    ...
    def encrypt_label():
        ...
        text = Text(frame_string_op, height=1, width=18, bg="white", bd=5,
        font=("Lucida Bright", 30, 'bold'),
        wrap=WORD)
        msg2 = Label(frame_string_op, text="Encrypted String :", font=('LucidaBright', 25, 'bold'), bg="AntiqueWhite1")
    ...
    def decrypt_label():
        ...
        font=("Lucida Bright", 30, 'bold'),
        wrap=WORD)
        msg2 = Label(frame_string_op, text="Decrypted String :", font=("LucidaBright", 25, 'bold'), bg="AntiqueWhite1")
...
def file_frame():
    ...
    button_encrypt = Button(frame_file_op, text="ENCRYPT", bg="white",
    height=2, width=20, bd=5,
    font=('Lucida Bright', 20, 'bold'),command=lambda: encrypt_file()) # command =lambda:encrypt_file())
    button_decrypt = Button(frame_file_op, text="DECRYPT", bg="white",
    height=2, width=25, bd=5,
    font=('Lucida Bright', 20, 'bold'),command=lambda: decrypt_file()) # command =lambda:decrypt_file())
    butt_exit = Button(frame_file_op, text="EXIT", bg="red", height=2,width=15, bd=5,font=('Lucida Bright', 20, 'bold'),command=lambda: raise_frame(main_frame(),frame_file_op))
    title2 = Label(frame_file_op, text="Select a file to :", font=('LucidaBright ', 30, 'bold'), bg="peach puff")
